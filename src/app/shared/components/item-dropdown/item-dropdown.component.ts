...
import { DropDownFooterComponent } from '../drop-down-footer/drop-down-footer.component';
...
export class ItemDropdownComponent implements OnInit, DropDownComponent, OnChanges, OnDestroy {
  ...
  @Input() isForAdvanceProduct : boolean = false ;
  ...
  @Output() CloseAddEquivvlenceProduct = new EventEmitter<any>();
  ...
  @Input() idCostumerFromGarage: number;
  @Input() allowRelationSupplierItems: boolean;
  @Input() isAdvancePaymentNature : boolean;
  ...
  prepareFiltersItemDropdown(): void {
    ...
    if (this.warehouseAssociated && !this.isForGarage && this.documentType == this.documentEnumerator.PurchaseAsset || this.isForSales) {
    ...
    this.filtersItemDropdown.isAdvancePaymentNature = this.isAdvancePaymentNature;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if(this.isForPurchase || this.isForSales){
      if(changes.warehouseAssociated && changes.warehouseAssociated.currentValue > 0){
        this.initDataSource();
      }
    }
    else {
  ...
  private notMultiSelectDropDown(paging) {
    this.filtersItemDropdown.idCustomer = this.idCostumerFromGarage;
  ...
  onSearchClicked(): void {
    ...
    if (this.activeKeyPress) {
      filterObject = {
        ...
        isForAdvanceProduct: this.isForAdvanceProduct
      };
    }
  ...
  onCloseSearchFetchModal(data): void {
    ...
    if(this.isForAdvanceProduct){
      this.CloseAddEquivvlenceProduct.emit();
    }
    this.clearValueOfSearch();
    this.onValueMapper(this);
